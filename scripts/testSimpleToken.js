...
async function transferTokens(from, to, amount, contract) {
  ...
  console.log(
    `Transferring ${amount} MST from ${from.address} to ${to.address}...`
  );
  const tx = await measureTime("Creating tx", async () =>
    contract.connect(from).transfer(to.address, amount)
  );

  // Wait for the transaction to be confirmed and included in the chain.
  console.log(`Waiting for tx ${tx.hash} to be mined...`);
  const receipt = await measureTime("Waiting tx to be mined", () => tx.wait());
  ...
  console.log(
    `    Gas Used      : ${receipt.gasUsed} (${receipt.effectiveGasPrice} wei / gas)`
  );
  ...
  console.log(
    `Found Transfer event: ${eventFrom} => ${eventTo}: ${eventValue}`
  );
...
async function main() {
  ...
  console.log(
    `Constructed token contract: ${tokenName} (${tokenSymbol}), owner is ${owner}`
  );
