...
describe("SimpleToken contract", function () {
  ...
  describe("Transactions", function () {
    it("should transfer tokens between accounts", async function () {
      const { simpleToken, owner, first_address, second_address } = await
        loadFixture(deploySimpleTokenFixture);

      expect(await simpleToken.balanceOf(first_address.address))
        .to.equal(0);
      expect(await simpleToken.balanceOf(second_address.address))
        .to.equal(0);
      ...
      await expect(simpleToken.transfer(first_address.address, 25))
        .to.changeTokenBalances(simpleToken, [owner, second_address], [-25, 25]);
    ...
    it("should emit Transfer events", async function () {
      ...
      await expect(simpleToken.transfer(first_address.address, 25))
        .to.emit(simpleToken, "Transfer").withArgs(owner.address, first_address, 25);

      await expect(simpleToken.connect(first_address).transfer(second_address, 25))
        .to.emit(simpleToken, "Transfer").withArgs(first_address.address, second_address.address, 25);
    });

    it("should fail if sender doesn't have enough tokens", async function () {
      const { simpleToken, owner, first_address } = await loadFixture(deploySimpleTokenFixture);
      const initialOwnerBalance = await simpleToken.balanceOf(owner.address);


      await expect(simpleToken.connect(first_address).transfer(owner.address, 1))
